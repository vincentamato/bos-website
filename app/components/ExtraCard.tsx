...
export default function ExtraCard({ extra }: ExtraCardProps) {
  const thumbnailUrl = extra.thumbnail 
    ? urlForImage(extra.thumbnail)?.toString() 
    : null

  return (
    <Link 
      href={`/shows/${extra.showSlug}/extras/${extra.slug}`} 
    ...
    >
      ...
      <div className="aspect-video relative overflow-hidden rounded-t-lg z-10">
        ...
        {/* Type badge */}
        <div className="absolute top-3 left-3 bg-white/20 backdrop-blur-sm text-white text-xs font-medium px-2 py-1 rounded-md">
          Extra
        </div>
      ...
      <div className="p-5 flex flex-col flex-grow relative z-10">
        <h3 className="text-xl font-bold text-white group-hover:text-white transition-colors duration-300">{extra.title}</h3>
