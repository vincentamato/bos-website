import { getExtraBySlug } from '@/sanity/lib/server'
...
export default async function ExtraPage({ params }: ExtraPageProps) {
  ...
  // If extra doesn't exist or belongs to a different show, return 404
  if (!extra || extra.showSlug !== slug) {
    notFound()
  }
    ...
    <div className="min-h-screen bg-black text-white">
      ...
      <div className="w-full max-w-[1600px] mx-auto pt-8 md:pt-12">
        <div className="relative pb-2 md:pb-4">
          <VideoPlayer 
            playbackId={extra.muxPlaybackId} 
            content={extra} 
            contentType="extra" 
          />
      ...
      <div className="container mx-auto px-4 py-6">
        ...
        <div className="flex items-center text-xs text-gray-400 mb-4">
          ...
          <Link href={`/shows/${extra.showSlug}`} className="hover:text-white transition-colors">{extra.showTitle}</Link>
          <span className="mx-1.5">•</span>
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-5">
          <div>
            <h1 className="text-2xl md:text-3xl font-bold">{extra.title}</h1>
          ...
          <div className="flex space-x-3 mt-4 md:mt-0">
            <ShareButton 
              title={`${extra.showTitle}: ${extra.title} (Extra)`} 
            />
          </div>
        </div>
        ...
        {extra.description && (
          <div className="bg-[#0A1428]/70 backdrop-blur-md rounded-xl p-5 border border-[#1A2A48]/40 mb-8 shadow-xl relative overflow-hidden">
            ...
            <div className="relative z-10">
              <h2 className="text-lg font-medium mb-3">About This Extra</h2>
              <p className="text-gray-300 text-sm leading-relaxed">{extra.description}</p>
            </div>
          </div>
        )}
      </div>
